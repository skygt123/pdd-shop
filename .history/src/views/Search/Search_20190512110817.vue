    ...
    <div class="shop">
      <cube-scroll-nav
        ...
        <cube-scroll-nav-panel
          ...
          :title="item.name">
          <ul>
            <li v-for="(food,index) in item.items" :key="index" style="border-bottom: 2PX solid red">
              {{item}}
              <div>
                <!-- <img :src="food.icon">
                <p>{{food.title}}</p> -->
                <!-- <div>{{this.$store.state.searchgoods}}</div> -->
                <!-- <div>dd:{{$store.state.searchgoods}}</div> -->
                <!-- <div>goods:{{this.$store.state.goods}}</div> -->
              </div>
            </li>
          </ul>
        </cube-scroll-nav-panel>
...
export default {
  ...
  data () {
    return {
      data: goods,
      current: this.searchgoods[0].tiltle
      // goodlist: {}
    }
  },
  created () {
    // this.$store.dispatch('getListAction')  //调用vuex 中的 getListAction异步方法
    this.$store.dispatch('reqSearchGoods') // 请求搜索数据
  },
  mounted () {
